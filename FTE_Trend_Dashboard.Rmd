...
service_list <- list("Perioperative Services","Radiology","Support Services","Nursing","Emergency Department")
...
service_line <- function(hosp,service, startdate = as.Date("12/22/2019",format="%m/%d/%Y")){
  ...
  service_line_graphly <- ggplotly(service_line_graph,
         tooltip=c("group","x","y")) %>%
  layout(#legend = list(orientation = "h",y=-.2,xanchor = "bottom")
    title = list(xanchor = "center"))
...
#Kable
k <- function(hosp,startdate = as.Date("12/22/2019",format="%m/%d/%Y")){
  library(tidyr)
  data <- System_Summary %>%
    filter(PP.END.DATE < as.Date("3/1/2020",format="%m/%d/%Y") |
           PP.END.DATE >as.Date("5/9/2020",format="%m/%d/%Y"),
           ...
           !is.na(SERVICE.LINE),
           PAY.CODE.MAPPING %in% worked) %>%
    group_by(SERVICE.LINE,DEFINITION.CODE,DEFINITION.NAME,PP.END.DATE) %>%
    summarise(FTE = round(sum(HOURS, na.rm = T)/75,2)) %>%
    mutate(DEPARTMENT = paste(DEFINITION.CODE,"-",toupper(DEFINITION.NAME)),
           DATES = as.character(PP.END.DATE)) %>%
    pivot_wider(id_cols = c(SERVICE.LINE,DEPARTMENT),names_from = DATES,values_from = FTE)
  colnames(data)[1] <- "Service Line"
  Ktable <<- data
}
...
#lapply(service_list,function(x)service_line(hosp="MSH",service=x))
service_line(hosp = "MSH",service = "Perioperative Services")
service_line(hosp = "MSH",service = "Radiology")
service_line(hosp = "MSH",service = "Support Services")
service_line(hosp = "MSH",service = "Nursing")
service_line(hosp = "MSH",service = "Emergency Department")
```
